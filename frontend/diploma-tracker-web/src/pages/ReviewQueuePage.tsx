...
import type { Group, ReviewQueueItem } from '../api/types'
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const data = await getReviewQueue(groupId || undefined, late)
        if (!isCurrent) return
        setItems(data)
      } catch (err) {
        if (!isCurrent) return
      ...
      <Card>
        <div className="mb-4 flex flex-wrap items-end gap-3">
          <div className="max-w-xs flex-1">
            <Select label={t('review.group')} value={groupId} onChange={setGroupId} options={groupOptions} />
          </div>
          <SegmentedControl
            ariaLabel={t('review.lateFilterLabel')}
            value={lateFilter}
            onChange={(value) => setLateFilter(value as LateFilter)}
            options={lateOptions}
          />
        </div>
